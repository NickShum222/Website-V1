    ...
    className={`w-full border-b-[1px] border-[#d9d9d9] flex flex-col justify-center items-center ${index === 0 ? "md:-mt-6 xl:pb-5 pb-3" : "xl:py-5 py-3"}  `}>
      <div className="w-full flex flex-row justify-between items-center md:pb-3 sm:pb-2 pb-1">
        <div className="flex flex-col justify-start items-start">
          <p className="transition-all duration-100 text-white lg:text-[30px] md:text-[26px] text-[25px] md:leading-[26px] leading-[24px] lg:leading-[30px]">{title}</p>
          <p className="transition-all duration-100 text-semiwhite lg:text-[20px] md:text-[18px] text-[18px] italic">{position}</p>
        </div>
        <div className="transition-all duration-100 text-[#BFBFBF] lg:text-[21px] md:text-[20px] text-[17px]">{date}</div>
      </div>
      <div className="transition-all duration-100 text-[#9c9c9c] lg:text-[17px] md:text-[15px] text-[14px]">{description}</div>
